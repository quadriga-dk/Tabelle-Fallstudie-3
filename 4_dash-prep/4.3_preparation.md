...
Sofern Sie Kapitel 1 und 3 nicht bearbeitet haben, wiederholen Sie bitte an dieser Stelle kurz [Kapitel 1.2](https://quadriga-dk.github.io/Tabelle-Fallstudie-3/pr%C3%A4ambel/technische_voraussetzungen.html), damit Sie ihr Setup für dieses und spätere Kapitel korrekt eingestellt haben. Erstellen Sie im Projektverzeichnis einen Ordner names `data/` und legen Sie dort die folgenden 2 Dateien ab:
...
**Vorbereitete Daten (CSV)**
*(Wichtig: Da diese Datei >100MB groß ist, wird sie im Repository über Git LFS verwaltet. Wenn Sie das Repository als ZIP heruntergeladen haben, ist die Datei auf Ihrem Rechner aktuell fehlerhaft/unvollständig. Bitte laden Sie die echte CSV-Datei über den folgenden Link herunter und ersetzen Sie sie im Ordner.)*
<a href="https://media.githubusercontent.com/media/quadriga-dk/Tabelle-Fallstudie-3/refs/heads/main/data/df_merged_final.csv" class="download-link" target='_blank'>⬇️ Download: Vorbereitete Daten (CSV)</a>
```
````

Die Grundlage für unser Dashboard bildet eine CSV-Datei der Nutzungsdaten aus dem bereits erwähnten Projekt Gieß den Kiez, mit welcher Sie bereits in Kapitel 3 gearbeitet haben. Diese muss zunächst eingelesen und bereinigt werden. Verwenden Sie dazu folgenden Code; der Text hinter dem `#` zeigt Ihnen an, was die darunter folgenden Codezeilen tun:
