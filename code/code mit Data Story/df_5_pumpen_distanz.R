...
df_merged_sum <- read.csv("data/df_intervall.csv", sep = ";", stringsAsFactors = FALSE, fileEncoding = "UTF-8")
...
write.csv2(df_merged_sum, "data/df_letzt.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
#  df_merged_sum_mit_distanzen <- read.csv("data/df_merged_sum_mit_distanzen_gesamter_Baumbestand.csv", sep = ";", stringsAsFactors = FALSE, fileEncoding = "UTF-8")
  df_merged_sum_mit_distanzen <- read.csv("data/df_letzt.csv", sep = ";", stringsAsFactors = FALSE, fileEncoding = "UTF-8")
...
#  write.csv2(df_merged_sum_mit_distanzen, "data/df_merged_sum_mit_distanzen_mit_umkreis_baumbestand.csv", row.names = FALSE)
  ...
  # Assuming your target geometry is in `df_merged_sum_mit_distanzen_mit_umkreis`
  # and pumps are in an sf object called `pumps_sf` (POINT geometry)
  
  # Make sure both have same CRS
  df <- st_transform(df_merged_sum_mit_distanzen_mit_umkreis, crs = 32633)  # or whatever matches your pumps
  pumps <- st_transform(pumps_sf, crs = st_crs(df))
  
  # Count pumps within 100m
  pump_counts <- st_is_within_distance(df, pumps, dist = 100)
  df$pumpen_im_umkreis_100m <- lengths(pump_counts)
  
  # Then mutate the category
  df <- df %>%
