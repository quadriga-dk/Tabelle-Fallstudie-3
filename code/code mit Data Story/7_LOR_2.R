...
df_merged$lng <- as.numeric(df_merged$lng)
df_merged$lat <- as.numeric(df_merged$lat)

# Remove rows with NA in lng/lat
df_merged_clean <- df_merged[complete.cases(df_merged[, c("lng", "lat")]), ]

# Convert to sf
df_merged_sf <- st_as_sf(df_merged_clean, coords = c("lng", "lat"), crs = 4326)
...
df_merged_mit_lor <- st_join(df_merged_sf, lor[, c("bzr_id", "bzr_name")], left = TRUE)
